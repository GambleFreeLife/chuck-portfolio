...
export function AuditLeadForm() {
  ...
  return <form className="audit-form" method="post" action="/api/audit" onSubmit={handleSubmit} aria-busy={status === "submitting"}>
    ...
    <label className="audit-field"><span>A little about your project <small>Required</small></span><textarea name="problem" minLength={10} maxLength={2000} rows={3} value={form.problem} onChange={e => update("problem", e.target.value)} placeholder="What does your business do, and what would you like help with?" required /></label>
