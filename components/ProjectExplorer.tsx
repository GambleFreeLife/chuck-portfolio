...
export function ProjectExplorer() {
  ...
  return <div className={s.explorer}>
    <div className={s.explorerToolbar}><div className={s.viewButtons} role="group" aria-label="Choose a project preview">{views.map((item, i) => <button type="button" key={item.name} aria-pressed={i === selected} aria-controls="project-preview" onClick={() => { setSelected(i); trackEvent("project_preview", { location: i === 0 ? "homepage" : "service_page" }); }}>{item.name}</button>)}</div><span className={s.previewLabel}>{selected === 0 ? "Condensed project preview" : "Actual website screenshot"}</span></div>
    <div className={s.projectCanvas} id="project-preview"><a href={view.url} target="_blank" rel="noopener noreferrer" aria-label={`Visit Baryames Cleaners: ${view.name}`} data-track="project_click" data-location="project_preview"><div className={s.browserBar}><span aria-hidden="true">● ● ●</span><span>{view.url.replace("https://", "")}</span><span aria-hidden="true">↗</span></div>{selected === 0 ? <BaryamesPreview /> : <Image src={view.image} alt={view.alt} width={1348} height={926} sizes="(max-width: 760px) 90vw, 1000px" />}</a></div>
